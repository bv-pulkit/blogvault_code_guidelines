...
Use only Tabs for indentation. Make sure Single Tab is equivalent to 2 spaces.

=== Indentation [[spaces-indentation]]

Use single *Tab* per indentation level (aka soft tabs).
...
.Why Bother with 100 characters in a World of Modern Widescreen Displays?
****

A lot of people these days feel that a maximum line length of 100 characters is
...
in editors with the window width set to 100, even if the tool places a marker
...
*We avoid using Conditional Assignment*
****
...
The names of predicate methods (methods that return a boolean value) should have prefix attached to them.
User prefixing predicate methods with the auxiliary verbs such as `is`, `does`, `has`, or `can`.
Avoid using `?` at the end of a function name. 
...
class Person
  ...
  def can_basketball_player
    false
  end

  def does_likes_candy
    true
  end
end
----
...
* https://github.com/rubocop-hq/ruby-style-guide[Ruby Style Guide]
